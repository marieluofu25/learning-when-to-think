...
One command submits **four** training/eval jobs with dependencies:
...
- If the SFT JSONL (`sft_data_path` inside `configs/chpc_sft_3action.yaml`) is **missing**, `run-all` auto-builds prerequisites:
  - if rollouts JSONL exists, submit `build-sft-data` then `train-sft` (`afterok`);
  - if rollouts JSONL is also missing, submit `build-rollouts-data` -> `build-sft-data` -> `train-sft`.
...
- Paths/configs: `RUN_ALL_ROLLOUTS_CFG`, `RUN_ALL_ROLLOUTS_OUT`, `RUN_ALL_SFT_CFG`, `RUN_ALL_SFT_OUT`, `RUN_ALL_GRPO_CFG`, `RUN_ALL_GRPO_OUT`, `RUN_ALL_DPO_CFG`, `RUN_ALL_EVAL_CFG`, `RUN_ALL_EVAL_CKPT`, `RUN_ALL_EVAL_OUT`
...
### Build rollouts + SFT JSONL

Rollouts can be built explicitly:

```bash
bash chpc/run_chpc.sh build-rollouts-data \
  data/rollouts_grouped_math_Qwen2.5-Math-7B.jsonl \
  configs/generate_rollouts_qwen25.yaml
```

Optional rollouts env overrides passed to `scripts/generate_rollouts_pivot`:
`ROLLOUT_MODEL`, `ROLLOUT_SUBSET`, `ROLLOUT_NUM_SAMPLES`, `ROLLOUT_MAX_TOKENS`,
`ROLLOUT_TEMPERATURE`, `ROLLOUT_TP`, `ROLLOUT_GPU_MEM`, `ROLLOUT_SEED`.
...
- if both are missing, it submits `build-rollouts-data` then `build-sft-data`, then SFT.
